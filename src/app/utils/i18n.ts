// Translation system for multi-language support
export type Language = 'he' | 'en' | 'ru' | 'es';

export const translations = {
  he: {
    appName: 'לוח אירועים ושידורים',
    todayView: 'תצוגת יומית',
    calendarView: 'לוח שנה',
    upcomingEvents: 'הכנסים והאירועים הבאים',
    eventDetails: 'פרטי אירוע',
    selectLanguage: 'בחר שפה',
    today: 'היום',
    events: 'אירועים',
    noEvents: 'אין אירועים ליום זה',
    backToCalendar: 'חזרה ללוח',
    backToToday: 'חזרה להיום',
    duration: 'משך',
    location: 'מיקום',
    description: 'תיאור',
    multiDayEvent: 'אירוע מרובה ימים',
    day: 'יום',
    selectDate: 'בחר תאריך',
  },
  en: {
    appName: 'Events and Broadcasts Calendar',
    todayView: "Daily View",
    calendarView: 'Calendar',
    upcomingEvents: 'Conventions & Upcoming Events',
    eventDetails: 'Event Details',
    selectLanguage: 'Select Language',
    today: 'Today',
    events: 'Events',
    noEvents: 'No events for this day',
    backToCalendar: 'Back to Calendar',
    backToToday: 'Back to Today',
    duration: 'Duration',
    location: 'Location',
    description: 'Description',
    multiDayEvent: 'Multi-day Event',
    day: 'Day',
    selectDate: 'Select Date',
  },
  ru: {
    appName: 'Календарь событий и трансляций',
    todayView: 'Сегодня',
    calendarView: 'Вид календаря',
    upcomingEvents: 'Конгрессы и предстоящие события',
    eventDetails: 'Детали события',
    selectLanguage: 'Выбрать язык',
    today: 'Сегодня',
    events: 'События',
    noEvents: 'Нет событий на этот день',
    backToCalendar: 'Назад к календарю',
    backToToday: 'Назад к сегодня',
    duration: 'Продолжительность',
    location: 'Место',
    description: 'Описание',
    multiDayEvent: 'Многодневное событие',
    day: 'День',
    selectDate: 'Выбрать дату',
  },
  es: {
    appName: 'Calendario de Eventos y Transmisiones',
    todayView: 'Vista de Hoy',
    calendarView: 'Vista de Calendario',
    upcomingEvents: 'Congresos y Próximos Eventos',
    eventDetails: 'Detalles del Evento',
    selectLanguage: 'Seleccionar Idioma',
    today: 'Hoy',
    events: 'Eventos',
    noEvents: 'No hay eventos para este día',
    backToCalendar: 'Volver al Calendario',
    backToToday: 'Volver a Hoy',
    duration: 'Duración',
    location: 'Ubicación',
    description: 'Descripción',
    multiDayEvent: 'Evento de varios días',
    day: 'Día',
    selectDate: 'Seleccionar Fecha',
  },
};

export const languageNames: Record<Language, string> = {
  he: 'עברית',
  en: 'English',
  ru: 'Русский',
  es: 'Español',
};

export function useTranslation(language: Language) {
  return translations[language];
}