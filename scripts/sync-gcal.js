...
const DELAY_MS = 300; // sequential, ~3 events/sec → ~12 API calls/sec, well under quota
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (ids) {
        const langs = Object.keys(ids);
        if (FORCE && event.gcalEventIds) { updated++; } else { created++; }
        await pool.query('UPDATE events SET gcal_event_ids=$1 WHERE id=$2', [ids, event.id]);
        console.log(`  [ok] ${event.id}  ${event.date}  "${event.title?.he || event.title?.en}"  langs:${langs.join(',')}`);
      } else {
        failed++;
        console.log(`  [skip] ${event.id}  "${event.title?.he || event.title?.en}"  (no langs synced)`);
      }
    } catch (err) {
